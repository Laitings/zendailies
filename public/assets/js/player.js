...
(function () {
  const vid = document.getElementById("zdVideo");
  const fps = parseFloat(vid?.dataset.fps || "25") || 25;
  const btnFwd = document.getElementById("stepFwd");
  const btnBack = document.getElementById("stepBack");
  if (!vid || !btnFwd || !btnBack) return;

  const frameDur = 1 / fps;

  function step(delta) {
    // Pause playback to avoid fighting the player
    vid.pause();
    const target = Math.max(0, (vid.currentTime || 0) + delta);
    vid.currentTime = target;
  }

  btnFwd.addEventListener("click", () => step(frameDur));
  btnBack.addEventListener("click", () => step(-frameDur));

  // Optional keyboard shortcuts: ← / → arrows for frame step
...
// ---- Auto-fade for frame-step controls (hover + idle show/hide) ----
(function () {
  const frame = document.getElementById("playerFrame");
  const vid = document.getElementById("zdVideo");
  const bar = document.querySelector(".frame-controls");
  const btnFwd = document.getElementById("stepFwd");
  const btnBack = document.getElementById("stepBack");
  if (!frame || !vid || !bar || !btnFwd || !btnBack) return;

  let hideTimer = 0;
  const IDLE_MS = 1600; // feel free to tweak
  const HIDE_WHEN_PAUSED = false; // set true if you also want to hide while paused
  ...
  function hideBar() {
    // Don’t hide if paused and you want it visible
    if (!HIDE_WHEN_PAUSED && vid.paused) return;
  ...
  function scheduleHide() {
    clearTimeout(hideTimer);
    if (!vid.paused) {
      hideTimer = setTimeout(hideBar, IDLE_MS);
    }
  }

  // Show on pointer movement over the player area
  function onPointerActivity() {
    showBar();
  }
  frame.addEventListener("mousemove", onPointerActivity);
  frame.addEventListener("pointerdown", onPointerActivity);
  frame.addEventListener("pointerenter", onPointerActivity);

  // Hide on leave (only if playing)
  frame.addEventListener("pointerleave", () => {
    if (!vid.paused) hideBar();
  });
  ...
  vid.addEventListener("pause", () => {
    clearTimeout(hideTimer);
    if (!HIDE_WHEN_PAUSED) bar.classList.remove("controls-hidden");
  });
  vid.addEventListener("ended", () => {
    clearTimeout(hideTimer);
    bar.classList.remove("controls-hidden");
  ...
  // Ensure buttons bring the bar back (keyboard or click)
  [btnFwd, btnBack].forEach((b) => {
  ...
  // Also show on keyboard frame-step shortcuts if you added them
  document.addEventListener("keydown", (e) => {
  ...
  // Initial state: visible, then auto-hide if playing
  showBar();
})();
