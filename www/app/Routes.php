...
$router->group([new AuthGuard], function (\App\Http\Router $r) use (
    ...
    // --- ADD THESE FOR MOBILE PROFILE ---
    $r->get('/account/mobile',   [\App\Http\Controllers\AccountController::class, 'mobileEdit']);
    $r->post('/account/update', [\App\Http\Controllers\AccountController::class, 'mobileUpdate']);

    // --------------------------------------------------------
    // HOME
    // --------------------------------------------------------
    $r->get('/', [\App\Http\Controllers\DashboardController::class, 'index']);
    ...
    // Batch thumbnails for specific day view
    $r->get('/admin/projects/{projectUuid}/days/{dayUuid}/clips/thumbnails', [ProjectClipsController::class, 'batchThumbnails']);

    // Batch thumbnails for "all days" view
    $r->get('/admin/projects/{projectUuid}/clips/thumbnails', [ProjectClipsController::class, 'batchThumbnails']);

    // Project-level clips index
    ...
    $r->post('/admin/projects/{projectUuid}/days/{dayUuid}/clips/bulk_delete', [ProjectClipsController::class, 'bulkDelete']);
    ...
    // Player batch thumbnail endpoints
    $r->get('/admin/projects/{projectUuid}/player/days/thumbnails', [ClipPlayerController::class, 'batchDayThumbnails']);
    $r->get('/admin/projects/{projectUuid}/player/scenes/thumbnails', [ClipPlayerController::class, 'batchSceneThumbnails']);
    ...
    $r->post('/admin/projects/{projectUuid}/days/{dayUuid}/player/{clipUuid}', [ClipPlayerController::class, 'storeComment']);
    ...
    // INSERT THESE HERE (Accessible to all logged-in users, pending Controller filtering)
    $r->get('/admin/jobs',                [WorkerController::class, 'index']);
    $r->get('/admin/jobs/status-summary', [WorkerController::class, 'statusSummary']);
    $r->post('/admin/jobs/toggle',        [WorkerController::class, 'toggle']);
    $r->post('/admin/jobs/requeue/{id}',  [WorkerController::class, 'requeue']);
    ...
    // NEW: Batch action for Drag-and-Drop
    $r->post('/admin/projects/{projectUuid}/batch-members', [ProjectMembersController::class, 'batchProjectAction']);
    ...
    $r->group([new \App\Http\Middleware\GlobalAdminGuard], function (\App\Http\Router $r3) use ($projectsController) {
        ...
        // Deactivate/Reactivate (Admin + Superuser)
        $r3->post('/admin/projects/{id}/deactivate',  [$projectsController, 'deactivate']);
        $r3->post('/admin/projects/{id}/reactivate',  [$projectsController, 'reactivate']);

        // Delete (Superuser only - but controller enforces this)
        $r3->post('/admin/projects/{id}/delete',      [$projectsController, 'delete']);
    });
});
