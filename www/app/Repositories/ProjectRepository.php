...
class ProjectRepository
{
    ...
    public function create(string $title, string $code, string $status = 'active', float $aspectRatio = 1.78): string
    {
        $this->pdo->beginTransaction();
        try {
            $uuid = $this->pdo->query("SELECT UUID()")->fetchColumn();

            $sql = "INSERT INTO projects (id, title, code, status, default_aspect_ratio, created_at, updated_at) 
                VALUES (UUID_TO_BIN(:uuid, 1), :title, :code, :status, :aspect, NOW(), NOW())";
            ...
            $st->execute([
                ...
                'code'   => $code,
                'status' => $status,
                'aspect' => $aspectRatio // Bind the new value
            ]);
    ...
    {
        // FIX: Added default_aspect_ratio to the SELECT list
        $sql = "SELECT BIN_TO_UUID(id,1) AS id, title, code, status, 
                   default_aspect_ratio, -- ADD THIS LINE
    ...
    public function updateByUuid(string $uuid, string $title, string $code, string $status, float $aspectRatio): void
    {
        // 1. Enforce unique code (excluding this specific project)
    ...
    ");
        ...
        $this->pdo->beginTransaction();
        try {
            // 2. Perform the update including the new aspect ratio
            $stmt = $this->pdo->prepare("
            UPDATE projects
               SET title = :title_u,
                   code  = :code_u,
                   status = :status_u,
                   default_aspect_ratio = :aspect_u,
                   updated_at = NOW()
        ...
        ");
            ...
            $stmt->execute([
                ...
                ':code_u'   => $code,
                ':status_u' => $status,
                ':aspect_u' => $aspectRatio, // New binding
                ':uuid_u'   => $uuid,
            ]);
    ...
    /**
     * Add these methods to your ProjectRepository class
     * (App/Repositories/ProjectRepository.php)
     * 
     * These are compatible with your existing PDO-based repository
     */

    /**
     * Update project status (for deactivate/reactivate)
     */
    ...
    /**
     * Soft delete - mark as deleted but keep records for reference
     */
    public function softDelete(string $uuid, array $archivalOptions): void
    {
        $optionsJson = json_encode($archivalOptions);

        $stmt = $this->pdo->prepare("
        UPDATE projects 
        SET deleted_at = NOW(), 
            archival_options = :options,
            status = 'archived',
            updated_at = NOW()
        WHERE id = UUID_TO_BIN(:uuid, 1)
        LIMIT 1
    ...
    /**
     * Permanent delete - remove project and optionally related data from database
     * 
     * @param string $uuid Project UUID
     * @param bool $deleteMetadata If false, preserves clip metadata (scene, take, etc)
     * @param bool $deleteComments If false, preserves comments
     */
    public function permanentDelete(string $uuid, bool $deleteMetadata, bool $deleteComments): void
    {
        $this->pdo->beginTransaction();

        try {
            // If we want to preserve metadata or comments, we need selective deletion
            if (!$deleteMetadata || !$deleteComments) {
                // For selective preservation, we'd update clips to null certain fields
                // rather than deleting the rows entirely.
                // 
                // Example for preserving metadata but deleting media references:
                // UPDATE clips SET file_path = NULL, poster_path = NULL, waveform_path = NULL
                // WHERE day_id IN (SELECT id FROM days WHERE project_id = UUID_TO_BIN(:uuid, 1))
                //
                // For now, we'll just delete everything since FK cascades handle it.
                // You can implement selective preservation later if needed.
            }

            // The project has FK constraints with ON DELETE CASCADE, so deleting the project
            // will automatically cascade to:
            // - days (via fk_days_project)
            // - clips (via fk_clips_day)
            // - project_members (via fk_pm_project)
            // - sensitive_groups (via fk_sg_project)
            // - comments (if they have FK to clips/days)
