...
class ProjectMembersController
{
    ...
    public function batchProjectAction(string $projectUuid): void
    {
        $this->assertProjectAdminOrSuperuser($projectUuid);
        Csrf::validateOrAbort($_POST['csrf'] ?? null);
        ...
        if ($action === 'remove') {
            // Bulk Remove
            foreach ($uuids as $personUuid) {
                try {
                    $this->projects->removeMember($projectUuid, $personUuid);
                } catch (\Throwable $e) {
                    // Continue/Ignore errors for bulk ops
                }
            }
        } elseif ($action === 'add') {
            // Bulk Add
            ...
            $placeholders = implode(',', array_fill(0, count($uuids), 'UUID_TO_BIN(?,1)'));

            // FIXED QUERY: Join accounts_persons and accounts to get the email
            $sql = "
                SELECT a.email 
                FROM accounts a
            ...
            foreach ($emails as $email) {
                try {
                    // Default new batch members to 'reviewer', no admin, no download
                    $this->projects->addMemberByEmail($projectUuid, $email, 'reviewer', 0, 0);
                } catch (\Throwable $e) {
