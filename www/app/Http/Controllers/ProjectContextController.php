...
final class ProjectContextController
{
    public function enter(string $projectUuid): void
    {
        ...
        // Block regular users from disabled projects
        if ($project['status'] === 'disabled' && !$isSuper && !$isGlobalAdmin) {
            $_SESSION['flash_error'] = 'This project is currently disabled. Contact an administrator.';
            die("BLOCK TRIGGERED: You are a regular user trying to enter a disabled project.");
            exit;
        }

        // Authorization (remaining member check)
        if (!$isSuper) {
