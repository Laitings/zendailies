...
// Ensure $old has the fallback structure
$old = $old ?? [
    'title'  => $project['title'] ?? '',
    'code'   => $project['code'] ?? '',
    'status' => $project['status'] ?? 'active',
    'default_aspect_ratio' => $project['default_aspect_ratio'] ?? '1.78' // ADD THIS
];
$errors = $errors ?? [];
?>
            ...
            <div class="zd-col-sidebar">
                ...
                <div class="zd-panel">
                    ...
                    <div class="zd-panel-body">
                        ...
                        <div class="zd-field">
                            <label class="zd-label">Default Aspect Ratio</label>
                            <select class="zd-select font-mono" name="default_aspect_ratio">
                                <?php
                                // 1. Check 'old' (if form failed), 2. check DB value, 3. fallback to 1.78
                                $currentAR = $old['default_aspect_ratio'] ?? $project['default_aspect_ratio'] ?? '1.78';

                                $ratios = ['2.39', '2.00', '1.85', '1.78', '1.66', '1.33'];
                                foreach ($ratios as $r): ?>
                                    <option value="<?= $r ?>" <?= (float)$currentAR == (float)$r ? 'selected' : '' ?>>
                                        <?= $r ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
