...
<script type="module" src="/assets/js/batch-thumbnails.js?v=<?= time() ?>"></script>
            ...
            <div class="zd-actions" style="display:flex; gap:12px; align-items:center; flex-wrap: nowrap;">
                ...
                <div style="display:flex; gap:12px; align-items:center; flex-shrink: 0;" class="<?= $disabledClass ?>" <?= $disabledTitle ?>>
                    ...
                    <?php if (!$dayIsPublished): ?>
                        <button type="button" class="zd-btn zd-btn-primary" id="<?= $isDisabled ? '' : 'zd-publish-open' ?>">Publish day</button>
                    <?php else: ?>
                        <button type="button" class="zd-btn" id="<?= $isDisabled ? '' : 'zd-unpublish-open' ?>">Unpublish day</button>
                    <?php endif; ?>
                        ...
                        <?php if (!$rows): ?>
                            ...
                            <?php else: foreach ($rows as $idx => $r): ?>
                                ...
                                <tr id="clip-<?= htmlspecialchars($r['clip_uuid']) ?>"
                                    ...
                                    <td class="col-thumb thumb-cell" data-field="thumb">
                                        <div style="position: relative; width: 100px; height: 56px;">
                                            <?php if (!empty($r['poster_path'])): ?>
                                                <img class="zd-thumb" src="<?= htmlspecialchars($r['poster_path']) ?>" alt="" style="width: 100%; height: 100%;">
                                            <?php else: ?>
                                                <div class="zd-thumb-skeleton"></div>
                                            <?php endif; ?>

                                            <?php if ((int)($r['is_restricted'] ?? 0) === 1): ?>
                                                <div class="zd-sensitive-chip">
                                                    SENSITIVE
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <!-- Camera -->
                                    <td class="col-camera <?php if ($isPowerUser): ?>zd-editable<?php endif; ?>"
                                        <?php if ($isPowerUser): ?>data-edit="camera" <?php endif; ?>>
                                        <span><?= htmlspecialchars($r['camera'] ?? '') ?></span>
                                    </td>
                                            ...
                                            <?php if ($isPowerUser): ?>
                                                <div class="zd-actions-wrap">
                                                    ...
                                                    <div class="zd-actions-menu">
                                                        ...
                                                        <button type="button"
                                                            class="zd-actions-item"
                                                            data-action="gen-poster"
                                                            data-clip-uuid="<?= htmlspecialchars($r['clip_uuid']) ?>">
                                                            Generate poster
                                                        </button>
                                                        <button type="button"
                                                            class="zd-actions-item"
                                                            data-action="gen-waveform"
                                                            data-clip-uuid="<?= htmlspecialchars($r['clip_uuid']) ?>">
                                                            Generate waveform
                                                        </button>

                                                        <form action="/admin/projects/<?= $project_uuid ?>/days/<?= $rowDayUuid ?>/clips/<?= $r['clip_uuid'] ?>/delete" method="POST" onsubmit="return confirm('Delete this clip and all its files?');">
                                                            <input type="hidden" name="_csrf" value="<?= \App\Support\Csrf::token() ?>">
                                                            <button type="submit" style="all:unset; color:#d62828; cursor:pointer; width:100%; text-align:left; padding:10px 16px; font-size:13px;">Delete</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
...
<form id="zd-bulk-delete-form" method="post" action="/admin/projects/<?= $project_uuid ?>/days/<?= $day_uuid ?>/clips/bulk_delete">
    <input type="hidden" name="_csrf" value="<?= \App\Support\Csrf::token() ?>">
