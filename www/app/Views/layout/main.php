...
<head>
    ...
    <style>
        ...
        .zd-proj-title {
            ...
            /* Increase line-height and add vertical padding to prevent clipping */
            line-height: 1.4;
            padding: 4px 0;

            /* Ensure the container doesn't clip the ascenders */
...
<body class="<?= trim($bodyClass) ?>">
    ...
    <header class="zd-topbar">
        <div class="zd-topbar-left">
            ...
            <?php if ($isSuperuser || $isGlobalAdmin): ?>
                <a href="/admin/projects"
                    class="zd-toplink <?= str_starts_with($reqUri, '/admin/projects') ? 'active' : '' ?>">
                    Projects
                </a>
            <?php endif; ?>

            <?php if ($isGlobalAdmin || $isProjectAdmin): ?>
                <a href="/admin/users"
                    class="zd-toplink <?= str_starts_with($reqUri, '/admin/users') ? 'active' : '' ?>">
                    Users
                </a>
            <?php endif; ?>
        </div>
    ...
    ?>
        <div class="zd-subbar" role="navigation" aria-label="Project links">
            <nav class="zd-subnav">
                <?php
                $pid = $puuid ?? ($project['project_uuid'] ?? ($project_uuid ?? null));

                // Define exact base paths
                $daysBase   = "/admin/projects/$pid/days";
                $clipsBase  = "/admin/projects/$pid/clips";
                $playerBase = "/admin/projects/$pid/player";

                // Logic: Player is active if URL contains /player anywhere
                $isPlayerActive = strpos($reqUri, '/player') !== false;

                // Logic: Clips is active if URL contains /clips anywhere (but not in player)
                $isClipsActive = strpos($reqUri, '/clips') !== false && !$isPlayerActive;

                // Logic: Days is active if it starts with /days AND not in player AND not in clips
                $isDaysActive = str_starts_with($reqUri, $daysBase) && !$isPlayerActive && !$isClipsActive;
                ?>

                <a href="<?= htmlspecialchars($daysBase) ?>"
                    class="zd-sub-link <?= $isDaysActive ? 'is-active' : '' ?>">
                    Days
                </a>

                <a href="<?= htmlspecialchars($clipsBase) ?>"
                    class="zd-sub-link <?= $isClipsActive ? 'is-active' : '' ?>">
                    Clips
                </a>

                <a href="<?= htmlspecialchars($playerBase) ?>?pane=days"
                    class="zd-sub-link <?= $isPlayerActive ? 'is-active' : '' ?>">
                    Player
                </a>

                <?php if ($isSuperuser || $isProjectAdmin): ?>
                    <?php
                    $isMembersOrSecurityActive = str_starts_with($reqUri, "/admin/projects/$pid/members") ||
                        str_starts_with($reqUri, "/admin/projects/$pid/sensitive-groups");
                    ?>
                    <a href="/admin/projects/<?= htmlspecialchars($pid) ?>/members"
                        class="zd-sub-link <?= $isMembersOrSecurityActive ? 'active' : '' ?>">
                        Members
                    </a>
    ...
    <script>
        ...
        // --- NEW: Mobile Desktop-Mode Warning (Once per Session) ---
        // --- NEW: Mobile Desktop-Mode Warning (Once per Session) ---
        (function() {
            if (sessionStorage.getItem('zd_mobile_warning_dismissed')) {
                return;
            }

            const isTouch = window.matchMedia("(any-pointer: coarse)").matches;
            const isSmall = screen.width < 1300;

            if (isTouch && isSmall) {
                const overlay = document.createElement('div');
                overlay.style.cssText = 'position:fixed;inset:0;background:rgba(11,12,16,0.95);z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;text-align:center;animation:zdFadeIn 0.3s ease-out;';

                overlay.innerHTML = `
                <div style="width: 700px; max-width: 90vw; max-height: 90vh; overflow-y: auto; background:#13151b; border:2px solid #3aa0ff; padding:40px; border-radius:12px; box-shadow:0 20px 50px rgba(0,0,0,0.8);">
                    
                    <h3 style="color:#3aa0ff; margin:0 0 20px 0; font-size:32px; text-transform:uppercase; letter-spacing:0.05em; line-height:1.2;">
                        Mobile Detected
                    </h3>
                    
                    <p style="color:#e5e7eb; font-size:18px; line-height:1.6; margin-bottom:30px;">
                        You are viewing the <strong>Desktop Version</strong> on a mobile device.
                        <br><br>
                        Please turn off "Desktop Mode" in your browser settings for the best experience.
                    </p>
                    
                    <button id="zdDismissMobileWarn" style="width:100%; padding:20px; background:#3aa0ff; color:#0b0c10; border:none; border-radius:8px; font-weight:700; cursor:pointer; text-transform:uppercase; font-size:18px;">
                        I Understand
                    </button>
                </div>
            `;
                ...
                document.getElementById('zdDismissMobileWarn').onclick = function() {
                    sessionStorage.setItem('zd_mobile_warning_dismissed', '1');
    ...
    <footer class="zd-footer">
        Developed by OnionBag and The Oppermann &copy; 2025
    </footer>
