...
$stMarkRunning = $pdo->prepare("
    ...
    WHERE id=:id LIMIT 1
");
$stRefresh      = $pdo->prepare("SELECT * FROM jobs_queue WHERE id=:id");
$stProgress     = $pdo->prepare("UPDATE jobs_queue SET progress_pct=:p, last_heartbeat=NOW() WHERE id=:id LIMIT 1");
$stDone         = $pdo->prepare("UPDATE jobs_queue SET state='done', progress_pct=100, finished_at=NOW(), updated_at=NOW() WHERE id=:id LIMIT 1");
$stFail         = $pdo->prepare("UPDATE jobs_queue SET state='failed', attempts=attempts+1, finished_at=NOW(), updated_at=NOW() WHERE id=:id LIMIT 1");
$stCancelCheck  = $pdo->prepare("SELECT cancel_requested FROM jobs_queue WHERE id=:id");
$stSetCmd       = $pdo->prepare("UPDATE jobs_queue SET ffmpeg_cmd=:cmd, log_path=:log, updated_at=NOW() WHERE id=:id LIMIT 1");
...
$stRunningCount = $pdo->prepare("SELECT COUNT(*) FROM jobs_queue WHERE state = 'running'");
