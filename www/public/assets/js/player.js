document.addEventListener("DOMContentLoaded", function () {
      ...
      requestAnimationFrame(() => {
        ...
        if (activeClip) {
          activeClip.scrollIntoView({
            ...
            block: "center", // Only scrolls if it's actually off-screen
          });
        }
      });
    }
  ...
  // FIXED: Check if user is typing in an input field before triggering theater mode
  ...
  // -------------------------------------------------
  // Metadata & Comments persistence (Smart Logic)
  // -------------------------------------------------
  ...
  metaSections.forEach((det) => {
    ...
    if (saved !== null) {
      // 1. Prioritize User Choice (if they manually toggled it in this session)
      det.open = saved === "1";
    } else if (section === "comments") {
      // 2. Comments Logic: Open by default ONLY if comments actually exist
      const hasComments = !!det.querySelector(".zd-comment-item");
      det.open = hasComments;
    } else {
      // 3. Default state for everything else (Basic, Audio, Extended)
      det.open = false;
    }
...
(function () {
  ...
  // 1. DATA VARIABLES (Let-scoped for Ajax updates)
  let fpsNum = Number(vid.dataset.fpsnum) || 0;
  let fpsDen = Number(vid.dataset.fpsden) || 0;
  let fps =
    fpsNum > 0 && fpsDen > 0
      ? fpsNum / fpsDen
  ...
  let drop = Math.abs(fps - 29.97) < 0.02 || Math.abs(fps - 59.94) < 0.02;

  // 2. YOUR ORIGINAL MATH FUNCTIONS (Unchanged)
  ...
  // 3. START FRAMES (Calculated once per clip)
  let startFrames = drop
    ? tcToFrames_DF(startTC, fps)
    : tcToFrames_ND(startTC, fps);

  // 4. AJAX UPDATE HOOK (Requires 'let' variables above)
  window.updatePlayerTimecodeState = function () {
    fpsNum = Number(vid.dataset.fpsnum) || 0;
    fpsDen = Number(vid.dataset.fpsden) || 0;
    fps =
      fpsNum > 0 && fpsDen > 0
        ? fpsNum / fpsDen
        : Number((vid.dataset.fps || "").replace(",", ".")) || 25;

    // Fallback protection for the edge case where dataset is momentarily empty
    if (fps <= 0) fps = 25;

    startTC = (vid.dataset.tcStart || "00:00:00:00").trim();
    drop = Math.abs(fps - 29.97) < 0.02 || Math.abs(fps - 59.94) < 0.02;
  ...
  // 5. RENDER FRAME (The "Gatekeeper" Fix)
  const useRVFC = "requestVideoFrameCallback" in HTMLVideoElement.prototype;

  function renderFrame(timeSeconds) {
    // SECURITY CHECK: If FPS is 0 or invalid (scrubbing edge case), do nothing.
    // This prevents the "Divide by Zero" freeze without changing your math.
    if (!fps || fps <= 0) {
