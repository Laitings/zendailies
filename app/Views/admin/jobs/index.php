    ...
    <div class="zd-header">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; gap:16px; align-items:center;">
                <form method="post" action="/admin/jobs/toggle">
                    <button type="submit" class="zd-btn <?= $isPaused ? 'zd-btn-ok' : 'zd-btn-danger' ?>" style="font-size:11px; padding: 6px 12px;">
        ...
        <table class="zd-jobs-table">
            ...
            <tbody>
                <?php foreach ($jobs as $j): ?>
                    <tr>
                        <td><span class="zd-chip zd-chip-muted"><?= strtoupper($j['preset']) ?></span></td>
                        <td>
                            <div style="font-weight:700; font-size:11px; color:#3aa0ff; text-transform:uppercase;">
                                <?= htmlspecialchars($j['project_title'] ?? 'Global') ?>
                            </div>
                            <div style="font-size:13px;"><?= htmlspecialchars($j['file_name'] ?: 'System Task') ?></div>
                        </td>
                        <td>
                            <?php
                            $state = $j['state'];
                            ...
                            if ($state === 'done')    $cls = 'zd-chip-ok';
                            if ($state === 'failed')  $cls = 'zd-chip-warn';
                            ?>
                            <span class="zd-chip <?= $cls ?>"><?= htmlspecialchars($state) ?></span>
                        </td>
                        <td>
