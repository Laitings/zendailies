...
<script>
    function copySetupLink(token, btn) {
        // Generate the full URL based on current host
        ...
        navigator.clipboard.writeText(url).then(() => {
            ...
            const originalFilter = btn.querySelector('img').style.filter;

            btn.style.opacity = '1';
            btn.querySelector('img').style.filter = 'invert(48%) sepia(70%) saturate(500%) hue-rotate(80deg) brightness(100%) contrast(100%)'; // Greenish

            const tooltip = document.createElement('span');
            tooltip.innerText = 'Copied!';
            tooltip.style = 'position:fixed; background:#2ecc71; color:white; padding:2px 6px; border-radius:4px; font-size:10px; margin-left:10px; z-index:1000;';
            btn.parentElement.appendChild(tooltip);

            setTimeout(() => {
                tooltip.remove();
                btn.style.opacity = originalOpacity;
                btn.querySelector('img').style.filter = originalFilter;
            }, 1500);
        }).catch(err => {
            console.error('Failed to copy link: ', err);
            alert('Could not copy link. Token: ' + token);
        });
    }
