 ...
 * streams -progress output, updates encode_jobs.progress_pct + heartbeat,
...
function claimNextJob(PDO $pdo, string $workerId): ?array
{
    ...
    // Refresh view of row
    $r = $pdo->prepare("SELECT * FROM encode_jobs WHERE id=:id");
    $r->execute([':id' => $job['id']]);
...
while (!$stop) {
    ...
    $updCmd = $pdo->prepare("UPDATE encode_jobs SET ffmpeg_cmd=:cmd, log_path=:log, updated_at=NOW() WHERE id=:id LIMIT 1");
